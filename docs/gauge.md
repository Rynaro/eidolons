...
V4-22 adds `bash gauge/tests/calibration-anchors.sh` for T01–T10 strategy calibration anchors. Extends V4-21; missing mechanisms stay pending; null/inconclusive never flips a default.
V4-23 adds `bash gauge/tests/release-anchors.sh` for T01–T08 migration/release/retirement gate anchors. Fixture readiness never authorizes tag/merge/publication/archival.
...
## V4-22 strategy calibration and adaptation gates

```sh
eidolons gauge calibration-enable --project "/absolute/ação project"
eidolons gauge calibration-baseline --project "/absolute/ação project" --input /absolute/baseline.json
eidolons gauge calibration-arm --project "/absolute/ação project" --input /absolute/arm.json
eidolons gauge calibration-batch --project "/absolute/ação project" --input /absolute/batch.json
eidolons gauge calibration-cost --project "/absolute/ação project" --input /absolute/cost.json
eidolons gauge calibration-promote --project "/absolute/ação project" --input /absolute/promo.json
eidolons gauge calibration-stop --project "/absolute/ação project" --input /absolute/stop.json
eidolons gauge calibration-decide --project "/absolute/ação project" --input /absolute/decide.json
eidolons gauge calibration-adapt --project "/absolute/ação project" --input /absolute/adapt.json
eidolons gauge calibration-generalize --project "/absolute/ação project" --input /absolute/gen.json
eidolons gauge calibration-strategy --project "/absolute/ação project" --input /absolute/strategy.json
eidolons gauge calibration-offline --project "/absolute/ação project" --input /absolute/offline.json
```

Extends V4-21 evaluation with mechanism ablations, freeze-before-holdout calibration batches, cost-complete benefit claims, unpromotion of null/inconclusive results (no default flip), trial boundary stops, bound adaptive decisions, versioned adaptation proposals, forward-only generalization claims, promoted strategy revalidation, and offline proposal isolation. Missing mechanism packages stay pending/ineligible. Required slices `workflow-ablations` and `routing-calibration`; `optional-offline-adaptation` delivers R10 fixture isolation without production self-mod. No V4-18 / V4-23.

V4-22 additions use the [calibration CLI](../gauge/cmd/eidolons-gauge/calibration.go), [calibration contract](../gauge/internal/contract/calibration.go), [store](../gauge/internal/store/calibration.go), [controller](../gauge/internal/controller/calibration.go), and [spec/decision](../.spectra/changes/gauge-v4-22/spec.md). Local fixture anchors are recorded in the [V4-22 receipt](campaigns/gauge/receipts/V4-22.md). Hosted CI pending publication.

## V4-23 evidence-scoped migration, release, and optional retirement

```sh
eidolons gauge release-enable --project "/absolute/ação project"
eidolons gauge migration-attempt --project "/absolute/ação project" --input /absolute/migration.json
eidolons gauge migration-recover --project "/absolute/ação project" --migration mig-id
eidolons gauge release-candidate --project "/absolute/ação project" --input /absolute/candidate.json
eidolons gauge release-authorize --project "/absolute/ação project" --input /absolute/auth.json
eidolons gauge release-retire --project "/absolute/ação project" --input /absolute/retire.json
eidolons gauge release-hostcap --project "/absolute/ação project" --input /absolute/hostcap.json
eidolons gauge release-hostcap-invalidate --project "/absolute/ação project" --id hostcap-1 --version host-v2
eidolons gauge release-readiness --project "/absolute/ação project" --input /absolute/ready.json
eidolons gauge release-rollback --project "/absolute/ação project" --input /absolute/rollback.json
```

Fixture gates for migration stage/verify/switch (fresh/legacy/v3/dirty/interrupted; no force-integrity bypass), failed-migration recovery of previous install + user state, release candidates distinguishing docs-only vs code/behavior (smoke ≠ live evidence), promotion authorization that leaves behavioral defaults unchanged and never authorizes tag/merge/publication, retirement blocked by unmigrated supported consumers, host-version drift invalidating capability-catalogue qualification, separate correctness / managed-operation / performance readiness decisions (no composite green badge), and strategy/method rollback retaining lineage/evidence/obligations without stale acceptance or budget refill. Governance-only improvements are labeled as such. Optional experiments/clients may remain deferred with explicit labels. This package does **not** perform a real release, merge, tag, deploy, or archive.

V4-23 additions use the [release CLI](../gauge/cmd/eidolons-gauge/release.go), [migration CLI](../gauge/cmd/eidolons-gauge/migration.go), [release contract](../gauge/internal/contract/release.go), [store](../gauge/internal/store/release.go), [controller](../gauge/internal/controller/release.go), and [spec/decision](../.spectra/changes/gauge-v4-23/spec.md). Local fixture anchors are recorded in the [V4-23 receipt](campaigns/gauge/receipts/V4-23.md). Publication/tag/release explicitly blocked.

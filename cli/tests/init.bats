...
# ─── G-R2B-1: eidolons.yaml stamps real version (R2B-1) ──────────────────

@test "eidolons.yaml stamps real version" {
  setup_fake_git
  local nexus_ver
  nexus_ver="$(cat "$EIDOLONS_ROOT/VERSION")"

  run eidolons init --preset minimal --hosts claude-code --non-interactive
  # sync will fail due to fake git; manifest is written first.
  [ -f eidolons.yaml ]
  # Line 2 must contain the real nexus version, not the 1.0.0 fallback.
  local yaml_line2
  yaml_line2="$(sed -n '2p' eidolons.yaml)"
  [[ "$yaml_line2" =~ "Generated by eidolons v${nexus_ver}" ]]
  [[ ! "$yaml_line2" =~ "v1.0.0" ]] || [[ "$nexus_ver" == "1.0.0" ]]
}

# ─── R3: pointer_targets in eidolons.yaml (Round 3 / v1.7.0) ─────────────────

# R3-init-1: non-interactive init with --hosts claude-code writes pointer_targets.
@test "non-interactive skips pointer prompts: pointer_targets derived from hosts.wire (R3)" {
  setup_fake_git

  run eidolons init --preset minimal --hosts claude-code --non-interactive
  # sync fails on fake git; we only care about eidolons.yaml.
  [ -f eidolons.yaml ]
  # pointer_targets derived from claude-code → CLAUDE.md.
  grep -qF "pointer_targets:" eidolons.yaml
  grep -qF "CLAUDE.md" eidolons.yaml
}

# R3-init-2: non-interactive init with --hosts codex writes AGENTS.md as pointer_target.
@test "non-interactive init --hosts codex: pointer_targets=[AGENTS.md] (R3)" {
  setup_fake_git
...
@test "detect_vendor_files_on_disk: finds CLAUDE.md on disk (R3)" {
  touch CLAUDE.md
...
# R3-init-6: detect_vendor_files_on_disk returns nothing when no vendor files present.
@test "detect_vendor_files_on_disk: empty when no vendor files on disk (R3)" {
  result="$(bash -c "
    . '$EIDOLONS_ROOT/cli/src/lib.sh' >/dev/null 2>&1
    detect_vendor_files_on_disk
  " 2>/dev/null)"

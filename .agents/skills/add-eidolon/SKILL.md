...
These come from `AGENTS.md` in the nexus repo and from tooling contracts — violating them breaks CI or consumer installs:
...
1. **Required files** (CI enforces): `AGENTS.md`, `AGENTS.md`, `install.sh`, `agent.md`, `README.md`. If any are missing, add them in the Eidolon's own repo before touching the roster.
...
4. **Per-host dispatch files**. At minimum, when `Codex` is in `HOSTS_CSV`, write `.Codex/agents/<name>.md`. The nexus has a safety net (`cli/src/sync.sh:179`) that writes a stub if the installer doesn't, but installers should do this themselves.
...
9. **Smoke test locally**:
   ...
   TMPDIR=$(mktemp -d); bash install.sh --target "$TMPDIR/x" --hosts Codex --non-interactive --force

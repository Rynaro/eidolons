...
|---------|---------------------|----------------------|------|--------|
| **ATLAS**<br><sub>A→T→L→A→S</sub> | Maps an unfamiliar codebase without writing a single line. Evidence-anchored findings, read-only by construction. | Auditing a new repo, onboarding, before any change. | [Rynaro/ATLAS](https://github.com/Rynaro/ATLAS) | 1.11.0 |
| **SPECTRA**<br><sub>S→P→E→C→T→R→A</sub> | Turns a scout report or rough idea into a decision-ready spec — scoring rubrics, validation gates, GIVEN/WHEN/THEN stories. | Planning a feature before you build it. | [Rynaro/SPECTRA](https://github.com/Rynaro/SPECTRA) | 4.8.0 |
| **APIVR-Δ**<br><sub>A→P→I→V→Δ/R</sub> | Implements features in brownfield code — pattern-first, test-anchored, bounded failure-recovery loop. | Shipping the change SPECTRA planned. | [Rynaro/APIVR-Delta](https://github.com/Rynaro/APIVR-Delta) | 3.6.0 |
| **IDG**<br><sub>I→D→G</sub> | Synthesizes documentation from sessions, specs, and deltas — provenance-first, with `[GAP]` / `[DISPUTED]` markers. | Chronicling what you (or the team) just built. | [Rynaro/IDG](https://github.com/Rynaro/IDG) | 1.7.0 |
| **FORGE**<br><sub>F→O→R→G→E</sub> | Deliberates on ambiguous trade-offs and novel problems. Names alternatives, surfaces assumptions, returns verdict + confidence. | Two patterns apply and the choice isn't obvious. | [Rynaro/FORGE](https://github.com/Rynaro/FORGE) | 1.8.0 |
| **VIGIL**<br><sub>V→I→G→I→L</sub> | Forensic debugger for failures resistant to normal repair. Reproduction-gated, counterfactual-verified, dependency-graph-ranked. | Flaky test, heisenbug, or a regression you can't explain. | [Rynaro/VIGIL](https://github.com/Rynaro/VIGIL) | 1.5.0 |
| **Kupo**<br><sub>K→U→P→O</sub> | Low-effort executor — heavier Eidolons delegate quick, localized, verifier-backed micro-tasks (rename, import fix, lockfile bump, lint autofix). Patches an ephemeral sandbox, proves it with a real verifier, and PROPOSEs a verified patch for the parent to commit (never writes the real tree). | Offloading trivial localized edits to keep a planner/coder session lean. | [Rynaro/Kupo](https://github.com/Rynaro/Kupo) | 1.0.0 |
...
- **`eidolons model` — vendor-neutral model management.** Every Eidolon/class gets a criteria-chosen *suggested* and *default* model on a `light < standard < deep` tier ladder, user-calibratable per project. Vendor model strings are quarantined to `roster/model-profiles.yaml` (cortex stays vendor-free); ships **anthropic** + **openai** profiles, extensible by data. `eidolons model {list,show,use,profile,reset}` + a guided interactive picker resolve and write the concrete `model:` into host agent frontmatter (claude-code/codex; copilot/cursor no-op), with a `doctor --deep` D9 drift gate. See [`docs/model.md`](docs/model.md).

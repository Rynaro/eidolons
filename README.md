...
The first six are shipped agent specialists; **Kupo** (executor) is a seventh, *in construction* — a low-effort delegation target the others hand quick localized micro-tasks to. **CRYSTALIUM** is a different capability class — `memory`, the shared substrate underneath the roster rather than a pipeline stage. Versions and detailed handoff contracts live in [`roster/index.yaml`](roster/index.yaml) — the machine-readable source of truth.

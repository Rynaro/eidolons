...
**Date:** 2026-05-03
**Scope:** Frontier techniques for an EIDOLONS.md "central cortex" that routes prompts to a fixed roster of 5 specialized agents (ATLAS, SPECTRA, APIVR-Δ, IDG, FORGE) and defines an ELITE tier of harness capabilities.
...
| 11 | **Tree of Thoughts** | Yao et al., arXiv:2305.10601 (NeurIPS 2023) | Explicit tree search over thought-states with self-eval at each node. 74% Game-of-24 vs 4% CoT. | **MED** — overkill for routing; relevant as an ELITE-tier mode for FORGE. |
...
| 17 | **Test-time compute scaling** | OpenAI o1 (Sep 2024 vendor); s1, Muennighoff et al., arXiv:2501.19393 | Sequential CoT-extension and parallel best-of-N both scale; **parallel scales better than sequential** for o1-class. | **HIGH** — key evidence that ELITE = parallel fanout, not just longer thinking. |
| 18 | **Critical caveat on long CoT** | "Revisiting Test-Time Scaling of o1-like Models" arXiv:2502.12215 (ACL 2025) | Longer CoTs do **not** consistently improve accuracy; correct solutions are often shorter. Self-revision can degrade. | **HIGH** — bounds the ELITE design: don't blindly stretch thinking budget. |
...
| 23 | **Budget-aware reasoning** | BudgetThinker arXiv:2508.17196; Certainty-Guided Reasoning arXiv:2509.07820 | Dynamically extend or terminate thinking based on confidence/budget. CGR achieves higher accuracy at lower token cost than fixed-budget. | **MED** — applicable to FORGE and SPECTRA at ELITE tier. |
...
These are the techniques that should anchor the cortex spec. For each: how it maps to the roster, what ELITE capability it unlocks, and a concrete implementation hint.
...
- **ELITE unlock:** Multiple Eidolons can be triggered in parallel for the same query (orchestrator-workers).
...
- **Threshold:** τ tunable per ELITE tier (e.g., 0.6 standard, 0.8 ELITE — higher bar to act).
...
- **ELITE unlock:** When a query touches 2+ phases of the SDLC (e.g., "scout + spec + implement"), spawn ATLAS → SPECTRA → APIVR-Δ as a relay, not as a single mega-agent.
...
### 3.4 Orchestrator-worker fan-out (ELITE only)
- **Maps to:** ELITE tier exclusively. Lead cortex spawns N parallel Eidolon invocations; collects results; synthesizes.
...
### 3.6 Self-Consistency for hard decisions (FORGE ELITE)
...
- **Threshold:** N=3 standard ELITE, N=5 high-stakes (matches o1's 64-sample sweet spot scaled to budget).
...
### 3.7 Evaluator-Optimizer loop (SPECTRA / IDG ELITE)
...
| **Mixture-of-Agents aggregation** | Highest quality at the top end; well-evidenced lift on hard tasks. | ~N× cost; aggregator quality is the new bottleneck; overkill for routing. | Reserve for ELITE tier on hard queries. |
...
| **Tournament / voting** | Robust under noise; calibratable. | Needs a verifier; expensive. | ELITE tier only. |
...
2. **Stage 2 — Confidence gate + ELITE escalation.** If supervisor's top match is high-confidence and a single Eidolon suffices → dispatch (standard tier). If query spans multiple phases → relay (orchestrator-worker pattern, Eidolon → Eidolon handoff). If decision is high-stakes or low-confidence → ELITE tier (parallel fanout to ≥2 Eidolons, optional FORGE arbitration, evaluator-optimizer wrapping).
...
## 5. ELITE Tier Definition — Candidates Ranked by Evidence Strength

| Rank | ELITE capability | Evidence strength | Mechanism |
|------|------------------|-------------------|-----------|
...
| 5 | **Heterogeneous model selection per Eidolon** | **Strong** — X-MAS 8-47% lift; Stop-Overvaluing-MAD position | ELITE allows lead = Opus, workers = Sonnet (mirrors Anthropic research-system); could allow per-Eidolon model pinning. |
...
| 8 | **Reflexion-style memory propagation across roster** | **Medium-strong** — Reflexion NeurIPS 2023; APIVR memory skill is empirical evidence | Cross-Eidolon learning store; ELITE tier writes back; standard tier reads only. |
| 9 | **Tournament selection of plans** | **Medium** — generative verifier work, GenSelect | When multiple SPECTRA plans or APIVR diffs are produced, run pairwise comparison. Expensive — ELITE only. |
...
**Recommended ELITE definition (composite):** parallel fan-out + worktree isolation + evaluator-optimizer wrapping + confidence-gated termination + Reflexion memory. Optional add-ons: self-consistency for FORGE, verifier-cascade for APIVR/SPECTRA.

---

## 6. Open Gaps

1. **Quantitative tradeoff: "ELITE always" vs "ELITE on demand."** Anthropic's 15× token cost number applies to research workloads. We don't have data for code-spec workloads with the Eidolons roster. Spec author should set ELITE as opt-in (user flag or auto-trigger on complexity ≥ threshold), not always-on.
...
5. **MAD / debate efficacy on coding-agent tasks.** The strongest debate evidence is on factuality and math; coding-agent results are mixed. The Eidolons stack is mostly software-engineering — assume debate is **not** ELITE-default; surface as an optional mode for FORGE only.

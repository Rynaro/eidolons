...
Select additive schema-2 status namespaces over (a) a schema bump or (b) rewriting delivery.go. Challenge invented quota percentages, fictional teams, premature cancel done/refund, and silent GAMBIT passes — none are introduced.
...
Status projection layer extends V4-15 InspectSnapshot without replacing delivery. Shared contract `gauge-status@1`. Slice `core-cli` required; **`optional-GAMBIT` explicitly deferred** with fixture consumer proving contract conformance.

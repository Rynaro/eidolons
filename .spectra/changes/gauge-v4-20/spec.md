...
**Assignment slices:** `core-cli` required; `optional-GAMBIT` explicitly deferred (not a silent pass).
...
| V4-20-R05 | WHERE an optional client is installed, the client SHALL consume the same policy/status contract as the CLI. | **V4-20-T05** (fixture consumer; GAMBIT deferred) |
...
See [decision.md](decision.md). Schema-2 additive status namespaces. Reuse V4-15 InspectSnapshot / delivery state; V4-11 reservations; V4-12 cancel states. core-cli only; optional-GAMBIT explicitly deferred. No V4-21 / V4-16+.
